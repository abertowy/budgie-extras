...
namespace BudgieTestTimeApplet {
    ...
    private void create_windows (
        ...
        bool allmonitors = testtime_settings.get_boolean("allmonitors");
        ...
        foreach (Gdk.Monitor m in monitors) {
            if (m.is_primary())  {
                ...
                // secundary testtime windows are set according to args
    ...
    private void open_window(
        ...
        // call the desktop testtime window;
        string cmd = winpath;
        if (wname != null) {
    ...
    public class BudgieTestTimeSettings : Gtk.Grid {
        ...
        public BudgieTestTimeSettings(GLib.Settings? settings) {
            ...
            anchors = {"nw", "ne", "se", "sw"};
            curr_anchor = testtime_settings.get_string("anchor");
            anchorbuttons[
            ...
            serialbox.pack_start(seriallabel, false, false, 0);
            serialfontbutton = new FontButton();
            serialbox.pack_start(serialfontbutton, false, false, 0);
            var spacelabel5 = new Gtk.Label("");
            this.attach(spacelabel5, 1, 10, 1, 1);
        ...
        private void update_xy_gsettings (string newanchor) {
            ...
            */
            ...
            testtime_settings.set_string("anchor", curr_anchor);
            testtime_settings.set_int("xposition", newx);
            testtime_settings.set_int("yposition", newy);
        }
        ...
        private void set_newlinespacing (SpinButton button, string setting) {
            ...
            testtime_settings.set_int(setting, newval);
        }
        ...
        public void set_initialallmonitors () {
            bool newval = testtime_settings.get_boolean("allmonitors");
            allmonitors.set_active(newval);
            bool newautopos = testtime_settings.get_boolean("autoposition");
            allmonitors.set_sensitive(newautopos);
        }

        public void set_initialautopos () {
            bool newval = testtime_settings.get_boolean("autoposition");
        ...
        private void connect_widgets () {
            ...
            serialfontbutton.font_set.connect (() => {
                set_newfont(serialfontbutton, "serialfont");
            });
        ...
        private void toggle_allmonitors (ToggleButton button) {
            ...
            testtime_settings.set_boolean("allmonitors", val);
        ...
        private void toggle_autopos (ToggleButton button) {
            ...
            if (!val) {
                ...
                */
                ...
                testtime_settings.set_int("xposition", newx);
                testtime_settings.set_int("yposition", newy);
                testtime_settings.set_string("anchor", "se");
                // turns out we need to make it conditional, else it is
                // registered as a value change
                if (curr_allmons) {
                    testtime_settings.set_boolean("allmonitors", false);
                }
                allmonitors.set_active(false);
            }
            else {
            ...
            testtime_settings.set_boolean("autoposition", val);
        }
        ...
        private void set_initialcolor (ColorButton button, string setting) {
            ...
            currcolor.parse(testtime_settings.get_string(setting));
            button.set_rgba(currcolor);
        }

        private void set_initialdrag () {
            // get current settings from gsettinsg, set dragbutton label
            bool curr_draggable = testtime_settings.get_boolean("draggable");
        ...
        private void toggle_drag () {
            ...
            testtime_settings.set_boolean("draggable", !curr_draggable);
            if (curr_draggable) {
                dragbutton.set_label(dragposition);
                draghint.set_text("");
    ...
    public class Applet : Budgie.Applet {
        ...
        {
            return new BudgieTestTimeSettings(this.get_applet_settings(uuid));
        }
        ...
        public Applet(string uuid) {
            ...
            gdkdisplay = Gdk.Display.get_default();
            winpath = moduledir.concat(@"/testtime_desktop $uuid");
...
public void peas_register_types(TypeModule module){
    ...
    objmodule.register_extension_type(typeof(
        Budgie.Plugin), typeof(BudgieTestTimeApplet.Plugin)
    );
}